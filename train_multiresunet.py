...
from keras.layers import Input, Conv2D, MaxPooling2D, Conv2DTranspose, concatenate, Flatten, Reshape, BatchNormalization, Activation, add
...
def MultiResUNet(height,width):
    ...
    inputs = Input((height, width, 5))
    incption1 = inceptionBlock(unet_unit, inputs) 
...
def createModel(fold, modelName):
    ...
    model = MultiResUNet(height=128, width=128)
